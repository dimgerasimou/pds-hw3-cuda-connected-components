...
All GPU implementations follow an iterative atomic union-find approach. Initially, each vertex is assigned
a unique label equal to its global ID. During each iteration, vertices inspect their neighbors and perform
atomic union operations to merge component labels. Iterations continue until convergence, detected when no
label changes.
