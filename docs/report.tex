...
5\times\) over a sequential CPU baseline and peak throughputs approaching 1 billion edges per second on social
...
highest throughput. On \emph{Orkut}, throughput exceeds 880 million edges per second, corresponding to
...
\end{axis}
\end{tikzpicture}
\caption{Throughput across datasets (log scale). Social graphs approach $\sim 10^3$ Medges/s; MAWI exposes poor mapping choices (notably Thread-per-Vertex).}
